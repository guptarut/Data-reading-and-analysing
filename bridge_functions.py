...
def format_data(data: List[List[str]]) -> None:  
    ...
    for sub_list in data:
        ...
        bridge.extend([int(data.index(sub_list) + 1), sub_list[NAME_INDEX],\
                       sub_list[HIGHWAY_INDEX], float(sub_list[LAT_INDEX]),\
                      float(sub_list[LON_INDEX]), sub_list[YEAR_INDEX],\
                       sub_list[LAST_MAJOR_INDEX], sub_list[LAST_MINOR_INDEX],\
                        int(sub_list[NUM_SPANS_INDEX])])

        split_list = sub_list[SPAN_LENGTH_INDEX].split('=')
        for string in split_list:
            if ';' in string:
                span_list.append(float(string[0: string.index(';')]))
        if len(span_list) > int(sub_list[NUM_SPANS_INDEX]):
            span_list.pop(0)
        bridge.append(span_list)

        if sub_list[LENGTH_INDEX] == '':
            bridge.append(float(0))
        else:
            bridge.append(float(sub_list[LENGTH_INDEX]))

        bridge.append(sub_list[LAST_INSPECTED_INDEX])

        for bci in sub_list[BCIS_INDEX:]:
            if bci != '':
                bci_list.append(float(bci))
        bci_list.remove(bci_list[0])
        bridge.append(bci_list)

        data[data.index(sub_list)] = bridge
...
def find_closest_bridge(bridge_data: List[list], bridge_id: int) -> int:
    ...
    # TODO
    distance_list = []
    closest_bridge_ids_list = []
    for bridge in bridge_data:
        if bridge[ID_INDEX] != bridge_id:
            distance_list.append(get_distance_between(bridge, get_bridge\
                                                      (bridge_data, bridge_id)))
    for bridge in bridge_data:
        if bridge[ID_INDEX] != bridge_id:
            if get_distance_between(bridge, get_bridge\
                                    (bridge_data, bridge_id)) == min(distance_list):
                closest_bridge_ids_list.append(bridge[ID_INDEX])

    return min(closest_bridge_ids_list)
...
def find_bridges_in_radius(bridge_data: List[list], lat: float, long: float,
    ...
    """
    # TODO
    bridge_ids_in_radius = []

    for bridge in bridge_data:
        if calculate_distance(bridge[LAT_INDEX], bridge[LON_INDEX],\
                              lat, long) <= distance:
            bridge_ids_in_radius.append(bridge[ID_INDEX])

    return bridge_ids_in_radius
...
def get_bridges_with_bci_below(bridge_data: List[list], bridge_ids: List[int],
    ...
    for bridge in bridge_data:
        if (bridge[ID_INDEX] in bridge_ids) and (bridge[BCIS_INDEX][0]\
                                                 <= bci_limit):
            bci_limit_id.append(bridge[ID_INDEX])
...
def get_bridges_containing(bridge_data: List[list], search: str) -> List[int]:
    ...
    bridge_id_list = []
    for bridge in bridge_data:
        if search.casefold() in bridge[NAME_INDEX].casefold():
            bridge_id_list.append(bridge[ID_INDEX])

    return bridge_id_list

def compare_two_list(parent_list: List[int], comparing_list:
                     List[int]) -> List[int]:
    """Return a list of integers that are in parent_list but not in
    comparing_list.

    >>> compare_two_list([1, 2, 3, 4, 5], [2, 4])
    [1, 3, 5]
    """

    compared_parent_list = []
    for integer in parent_list:
        if integer not in comparing_list:
            compared_parent_list.append(integer)

    return compared_parent_list

def compare_three_list(parent_list: List[int], comparing_list1:
                       List[int], comparing_list2: List[int]) -> List[int]:
    """Return a list of integers that are in parent_list but not in
    comparing_list1 and comparing_list2.

    >>> compare_three_list([1, 2, 3, 4, 5], [2, 4], [1])
    [3, 5]
    """

    compared_parent_list = []
    for integer in parent_list:
        if (integer not in comparing_list1) and (integer not in comparing_list2):
            compared_parent_list.append(integer)

    return compared_parent_list

def compare_four_list(parent_list: List[int], comparing_list1:
                      List[int], comparing_list2: List[int], comparing_list3: \
                      List[int]) -> List[int]:
    """Return a list of integers that are in parent_list but not in
    compare_list1 , comparing_list2 and comparing_list3.

    >>> compare_four_list([1, 2, 3, 4, 5], [2, 4], [1], [2, 5])
    [3]
    """

    compared_parent_list = []
    for integer in parent_list:
        if (integer not in comparing_list1) and (integer not in comparing_list2) and\
           (integer not in comparing_list3):
            compared_parent_list.append(integer)
...
def assign_inspectors(bridge_data: List[list], inspectors: List[List[float]],
    ...
    """
    # TODO
    assigned_inspectors_list = []
    assigned_bridge_ids = []

    for inspector_position in inspectors:
        inspector_list = []
        
        high_priority_list = get_bridges_with_bci_below(bridge_data, \
                        find_bridges_in_radius(bridge_data, inspector_position[0], \
                        inspector_position[1], HIGH_PRIORITY_RADIUS), HIGH_PRIORITY_BCI)
        
        medium_priority_list = get_bridges_with_bci_below(bridge_data, \
                         find_bridges_in_radius(bridge_data, inspector_position[0], \
                      inspector_position[1], MEDIUM_PRIORITY_RADIUS), MEDIUM_PRIORITY_BCI)

        low_priority_list = get_bridges_with_bci_below(bridge_data, \
                          find_bridges_in_radius(bridge_data, inspector_position[0], \
                          inspector_position[1], LOW_PRIORITY_RADIUS), LOW_PRIORITY_BCI)

        corrected_high_priority_list = compare_two_list(high_priority_list,\
                                        assigned_bridge_ids)

        corrected_medium_priority_list = compare_three_list(medium_priority_list,\
                                    assigned_bridge_ids, corrected_high_priority_list)

        corrected_low_priority_list = compare_four_list(low_priority_list,\
                                      assigned_bridge_ids, corrected_high_priority_list,\
                                     corrected_medium_priority_list)

        for bridge_id in corrected_high_priority_list:
            if len(inspector_list) < max_bridges:
                inspector_list.append(bridge_id)
                assigned_bridge_ids.append(bridge_id)

        for bridge_id in corrected_medium_priority_list:
            if len(inspector_list) < max_bridges:
                inspector_list.append(bridge_id)
                assigned_bridge_ids.append(bridge_id)

        for bridge_id in corrected_low_priority_list:
            if len(inspector_list) < max_bridges:
                inspector_list.append(bridge_id)
                assigned_bridge_ids.append(bridge_id)

        assigned_inspectors_list.append(inspector_list)

    return assigned_inspectors_list
